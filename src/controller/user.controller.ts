...
// deno-lint-ignore no-explicit-any
export const signin = async ({request, response, cookies}: {request:any; response:any; cookies: any}) => {
  const body = request.body({ type: "json" })
  const result = await body.value
  const { username, password } = result
  if(!username) return fresponse(500, null, "Masukkan username anda", response)
  try {
...
// deno-lint-ignore no-explicit-any
export const updateUserById = async ({request, response, params}: {request:any; response:any; params:any}) => {
